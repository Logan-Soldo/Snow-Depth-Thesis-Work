...
def read_file(CellList,filelist,region_select,input_location):
    ...
    while True:
        ...
        if input_location == "1":
            ...
            try:   
                if region_select == "5":
                    for i in CellList:
                        temp_list = []
                        for cell in i:
                            flist = open("G:\School\Thesis\Research\SnowDepth_Data\Outputs\%s\SnowDepth\%s_%s" %(cell,cell,file))
                            temp_list.append(flist)
                        filelist.append(temp_list)
                else:
                    for cell in CellList:            
                        open("G:\School\Thesis\Research\SnowDepth_Data\Outputs\%s\SnowDepth\%s_%s" %(cell,cell,file))
                        filelist.append("G:\School\Thesis\Research\SnowDepth_Data\Outputs\%s\SnowDepth\%s_%s" %(cell,cell,file))
            ...
            file = input("Input a file name: ") 
            try:
                filelist = pd.read_fwf("G:\School\Thesis\Research\SnowDepth_Data\Outputs\All\PercentMiss\%s" %(file),header=None)                  
                return file,filelist
            except:
                print("file doesn't exist")
                break
...
def region_define(region_select):
    ...
    '''
    CellList = []
    reg_file = pd.read_fwf("G:\School\Thesis\Research\SnowDepth_Data\Outputs\All\PercentMiss\AllPercent_Depth.txt",header=None)
...
def Set_threshold(filelist,header,input3,selection,region_select):
    ...
    for fname in filelist:
        try:
            if input3 == "DaysAboveThreshold.txt":
                data= np.loadtxt((open(fname).readlines()), skiprows=1, dtype=None)  # Coming out of loop too early, need to build a list of lists.
            else:
                data= np.loadtxt((open(fname).readlines()[:-1]), skiprows=1, dtype=None)  # Coming out of loop too early, need to build a list of lists.
           # print(data[0])
            data_list.append(data)
...
#            m_round = round(m,3)            
  ...
  #      print(PerROC)        
      #  sub[i].set_xlim(1966,2017)
        ...
        sub[i].set_xticks(range(1966,2018,5))#,fontsize=8)
        sub[i].set_xticklabels(range(1966,2018,5),fontsize=10) 
        sub[i].locator_params(axis="y",tight=True,nbins=10)   # change nbins integer to reduce number of y ticks
        sub[i].tick_params(axis="y",labelsize=10)                  # Needs to be changed for every plot.
        sub[i].xaxis.set_minor_locator(AutoMinorLocator(5))
        sub[i].set_ylabel(yaxis,fontsize=10,multialignment="center")
...
def savefig(figname):    
    plt.savefig("G:\School\Thesis\Research\\Figures\\%s.jpg" %(figname),dpi=600)  # Need to vary this formatting between graphics.
    plt.show()                    
