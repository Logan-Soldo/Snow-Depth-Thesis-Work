...
!	  integer station_num,station_10,station_25,station_50,station_100
	  ...
	  real 	Nov_Mean,Dec_Mean,AvgMaxCount,SumMaxCount,DecadeSum1,Decade
	  real  per25,per50,per100,per125,per150,tot_snwcover,countGT0(52)
	 ...
	 & outputfile12,outputfile13,outputfile14
...
write(*,nml=cdh_nml)
	  ...
	  Open(200,file=outputfile14, status = "replace")		 ! Junk
...
! Initialize months
	...
	! Monthly average will calculated within the loop							
							...
							! Checking for mean_depth values greater than 0.
									...
									if ((count_day .GE. 93) .and. (count_day .LE. 274)) then			! Beginning of October to end of March.						
										if (mean_depth .gt. 0) then
	...
	! Monthly average will calculated within the loop							
							...
							! Checking for mean_depth values greater than 0.
									...
									if ((count_day .GE. 93) .and. (count_day .LE. 304)) then
										...
										range_count(k) = range_count(k) + 1   ! Counting number of days between October 1st and end of April.
									endif
...
!							write(200,*) year,study_year,month,day,count_day,mean_depth,count25(k),count50(k),count76(k),count100(k),count125(k)    
...
! Converting count days into Julian Day (only if necessary).
	...
	AbsAverageElevation = 0
		...
		per50 = ((sum(count50,k))/(sum(range_count,k)))*100
		per100 = ((sum(count76,k))/(sum(range_count,k)))*100
...
!		write(200,*) i,j,latitude,longitude,sum(range_count,k),sum(count125,k)
		...
		write(80,3600) i,j,latitude,longitude,AbsAverageElevation,total_mpercent,miss_total,annPercent_76,annCounter_76,&
		...
		write(81,3700) i,j,latitude,longitude,sum_range,tot_snwcover,per25,per50,per100,per125,per150
...
! Write out the monthly averages here.
		...
		Decade = ((sum(countGT0(2:12))/(sum(counter_76))))*100				! Calculating number of days above 7.6cm by decade
		
		
		DecadeSum1 = ((sum(count76,k,mask=((k.ge.2).and.(k.le.12))))/(sum(range_count,k,((k.ge.2).and.(k.le.12)))))*100
		write(*,*) Decade
